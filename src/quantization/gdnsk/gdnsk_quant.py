...
from src.quantization.gdnsk.utils.model_helper import ModelHelper
from src.quantization.gdnsk.gdnsk_loss import PotentialLoss
from src.quantization.gdnsk.utils import model_stats
...
class gdnskQuant(BaseQuant):
    ...
    def quantize(self, lmodel: pl.LightningModule, in_place=False):
        ...
        qmodel.noise_ratio = gdnskQuant.noise_ratio.__get__(
            qmodel, type(qmodel))

        # Important step. Replacing training and validation steps
        # with alternated ones.
        if self.config.quantization.distillation:
            qmodel.training_step = gdnskQuant.distillation_noisy_training_step.__get__(
                qmodel, type(qmodel)
            )
        else:
            qmodel.training_step = gdnskQuant.noisy_training_step.__get__(
                qmodel, type(qmodel)
            )

        qmodel.validation_step = gdnskQuant.noisy_validation_step.__get__(
            qmodel, type(qmodel)
        )
        qmodel.test_step = gdnskQuant.noisy_test_step.__get__(
            qmodel, type(qmodel))
        ...
        if self.config.quantization.freeze_batchnorm:
            gdnskQuant.freeze_all_batchnorm_layers(qmodel)                
    ...
    def distillation_noisy_training_step(self, batch, batch_idx):
        ...
        outputs = gdnskQuant.noisy_step(self, inputs)
